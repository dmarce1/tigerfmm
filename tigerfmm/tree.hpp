...
public:
	...
	void compute_expansions(const expansion<real> &Lphi0, const space_vector<real> &X0);
	...
	void direct_interaction_at(const space_vector<real> &X, real &phi, space_vector<real> &g) const;
	real compute_error(const tree &root, bool torque);
	void initialize();
	integer create_tree(const space_vector<real> &xmin, const space_vector<real> &xmax, std::vector<particle> parts, integer lev);
	real solve();
	static real get_top10();
	static void set_theta(real theta);
	void save_analytic();
	bool load_analytic(FILE *fp = nullptr);
};
