...
void tree::output(const char *filename) const {
	if (is_leaf) {
...
bool tree::is_well_separated_from(const tree &other) const {
	real Z = zero;
	for (integer d = 0; d != NDIM; ++d) {
...
bool tree::get_leaf_directory(std::vector<tree*> &dir) {
	if (is_leaf) {
		return true;
	} else {
		for (integer i = 0; i != nchild; ++i) {
...
void tree::compute_interactions(std::vector<tree*> cells) {
	static std::vector<std::vector<tree*>> __child_cells(100);

	std::vector<tree*> &child_cells = __child_cells[level];
...
void tree::direct_interaction_at(const space_vector<real> &X, real &phi, space_vector<real> &g) const {
	if (level == 0) {
		for (integer d = 0; d != NDIM; ++d) {
			g[d] = zero;
		}
...
real tree::compute_error(const tree &root, bool torque) {
	if (level == 0) {
		top10.clear();
	}
...
void tree::initialize() {
	...
	printf("%i %i %i particles\n", int(these_parts.size()), int(n1), int(n2));

	create_tree(Xmin, Xmax, std::move(these_parts), 0);

}

integer tree::create_tree(const space_vector<real> &xmin, const space_vector<real> &xmax, std::vector<particle> parts, integer lev) {
	integer maxlev = lev;
	level = lev;
	Xmax = xmax;
	is_leaf = true;
	...
	if (particles.size() > ncrit) {
		...
		while (l != particles.end()) {
			const auto i = std::max(integer(0), std::min(integer(1), integer(real(2) * (l->X[0] - Xmin[0]) / (Xmax[0] - Xmin[0]))));
			const auto j = std::max(integer(0), std::min(integer(1), integer(real(2) * (l->X[1] - Xmin[1]) / (Xmax[1] - Xmin[1]))));
			const auto k = std::max(integer(0), std::min(integer(1), integer(real(2) * (l->X[2] - Xmin[2]) / (Xmax[2] - Xmin[2]))));
			child_parts[i][j][k].push_back(std::move(*l));
			++l;
		}
		for (integer i = 0; i < 2; ++i) {
			this_xmin[0] = Xmin[0] + real(0.5) * real(i) * (Xmax[0] - Xmin[0]);
			this_xmax[0] = this_xmin[0] + real(0.5) * (Xmax[0] - Xmin[0]);
			for (integer j = 0; j < 2; ++j) {
				this_xmin[1] = Xmin[1] + real(0.5) * real(j) * (Xmax[1] - Xmin[1]);
				this_xmax[1] = this_xmin[1] + real(0.5) * (Xmax[1] - Xmin[1]);
				for (integer k = 0; k < 2; ++k) {
					this_xmin[2] = Xmin[2] + real(0.5) * real(k) * (Xmax[2] - Xmin[2]);
					this_xmax[2] = this_xmin[2] + real(0.5) * (Xmax[2] - Xmin[2]);
					maxlev = std::max(children[i * 4 + j * 2 + k]->create_tree(this_xmin, this_xmax, std::move(child_parts[i][j][k]), lev + 1), maxlev);
				}
			}
		}
	}
